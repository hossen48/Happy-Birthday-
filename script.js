...
let screen=0, musicOn=false, audioCtx=null;
...
function startMusic(){
  ...
  $('#musicLabel').textContent='soundtrack on · medium ♫';
}
function toggleMusic(){
  ...
  if(audio.paused){
    ...
    $('#musicLabel').textContent='soundtrack off · tap to play';
  }
}
$('#musicBtn').addEventListener('click',e=>{e.stopPropagation();toggleMusic()});document.addEventListener('pointerdown',e=>{if(!musicOn && !e.target.closest('#musicBtn'))startMusic()},{once:true});
...
$('#blowBtn').addEventListener('click',()=>{
  ...
  $('#wishMsg').textContent='Fwoooosh… wish released into the universe. ✨💗';
  $('#blowBtn').textContent='Wish sent ♡';
  sparkleBurst(34);
  setTimeout(()=>go(3),1800);
});
...
function bindLetter(){
  const stage=$('#letterEnvelopeStage'), envelope=$('#letterEnvelope'), paper=$('#actualLetter');
  ...
  const open=()=>{
    ...
    setTimeout(()=>{
      paper.classList.remove('letter-paper-hidden');
      paper.classList.add('letter-paper-reveal');
      sparkleBurst(16);
      setTimeout(()=>stage.classList.add('finished'),650);
    },720);
  };
  envelope.addEventListener('click',open);
  envelope.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}});
  // Give her a moment to see the sealed envelope before inviting the reveal.
  setTimeout(()=>stage.classList.add('ready'),350);
}

function bindReasons(){
  const data=[
    ['THE SMILE FILE','Your beautiful smile. The kind that makes Hossen forget what he was saying.'],
    ['THE KITCHEN FILE','Your cooking. Evidence that you can feed me and emotionally destroy my standards for normal food at the same time.'],
    ['THE HAIR FILE','The smell of your hair. Yes, this is oddly specific. No, I will not be taking questions.'],
    ['THE FACE FILE','Your funny faces. Honestly, your facial expressions deserve their own documentary.'],
    ['THE ANGER FILE','Your anger. It annoys me. I secretly love it. Terrible design flaw in my personality.'],
    ['THE CARE FILE','The way you care. Quiet things, little things, the things that make a person feel looked after.'],
    ['THE TIME FILE','The time you give me. I notice the time you choose to give me.'],
    ['THE BONK FILE','The way you beat me. Filed under PLAYFUL COUPLE NONSENSE. 😂'],
    ['THE THREAT FILE','Your unserious threats. The relationship apparently installed its own comedy department.'],
    ['THE PEACE FILE','The way you handle my anger. Somehow you can deal with Hossen when Hossen.exe is overheating.']
  ];
  const grid=$('#reasonGrid'), title=$('#reasonTitle'), body=$('#reasonBody'), count=$('#reasonCount'), status=$('#reasonStatus'), next=$('#reasonsNext');
  $$('.reason-file').forEach(btn=>{
    const i=+btn.dataset.reason;
    const pics=[i*3+1,i*3+2,i*3+3];
    btn.innerHTML=`<span class="reason-file-front"><b>FILE ${String(i+1).padStart(2,'0')}</b><small>SEALED · SWAP ME</small></span>
      <span class="reason-file-back"><b>${data[i][0]}</b><small>${data[i][1]}</small><em>SWAPPED ♡</em></span>`;
    btn.addEventListener('click',()=>{
      const nextPicIndex=((btn.dataset.swapIndex||0)*1)%3;
      btn.dataset.swapIndex=String(nextPicIndex+1);
      const photoNo=pics[nextPicIndex];
      reasonSeen.add(i);
      btn.classList.add('opened','flipped');
      title.textContent=data[i][0];
      body.textContent=data[i][1];
      $('#reasonPhoto').src=`assets/photos/${String(photoNo).padStart(2,'0')}.webp`;
      count.textContent=reasonSeen.size;
      status.textContent=reasonSeen.size===10?'archive complete — all 30 memories accounted for 💗':'swap again or open another file, detective Adu 👀';
      if(reasonSeen.size===10){
        next.disabled=false;
        $('#handSecret')?.classList.add('unlocked');
        sparkleBurst(24);
      }else next.disabled=true;
    });
  });
  $('#holdHandBtn')?.addEventListener('click',()=>{
    const r=$('#handResponse');
    r.classList.add('show');
    $('#holdHandBtn').textContent='Still holding ♡';
    sparkleBurst(30);
    document.querySelector('.hand-secret')?.classList.add('held');
  });
}
function bindPhotoMuseum(){$$('.photo-open').forEach(btn=>btn.addEventListener('click',()=>{const box=$('#photoLightbox');if(!box)return;$('#lightboxImg').src='assets/photos/'+btn.dataset.photo;$('#lightboxCaption').textContent=btn.dataset.caption;box.classList.add('show');box.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';viewedPhotos.add(btn.dataset.photo);$('#galleryCount').textContent=viewedPhotos.size;$('#galleryStatus').textContent=viewedPhotos.size===30?'all 30 frames inspected — excellent taste 💗':'the curator is watching 👀';sparkleBurst(8)}))}
